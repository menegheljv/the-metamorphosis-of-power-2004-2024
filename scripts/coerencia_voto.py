...
Produces:
  - output/coerencia_voto.csv (per-precinct pct_prefeito_nosso, pct_vereador_nosso, ano)
  - output/chart_coerencia_voto.b64 (scatter, 2020 vs 2024, with trend lines)
"""
import pandas as pd
import numpy as np
import matplotlib
...
print(f"2020: n={len(df2020)} seções, correlação (Pearson r) = {r2020:.3f}")
print(f"2024: n={len(df2024)} seções, correlação (Pearson r) = {r2024:.3f}")
...
for ax, df, year, color, r in zip(axes, [df2020, df2024], [2020, 2024], [RED, GREEN_DEEP], [r2020, r2024]):
    ...
    ax.set_xlim(0, 100)
    ax.set_ylim(0, 100)
    ax.set_xlabel(f"% de votos válidos no candidato a prefeito do grupo, {year}", fontsize=10, color=INK)
...
fig.text(0.5, 0.99, "EM 2020, O VOTO PARA PREFEITO E VEREADOR ANDAVA JUNTO. EM 2024, MENOS.", fontsize=14.5, color=TITLE_GRAY, fontweight="bold", fontfamily="Anton", ha="center", va="top")
fig.text(0.5, 0.935, "Cada ponto é uma seção: votos válidos no prefeito vs. nos vereadores do grupo", fontsize=9.5, color=MUTED, fontfamily="Anton", ha="center", va="top")
