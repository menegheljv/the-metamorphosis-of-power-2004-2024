# -*- coding: utf-8 -*-
"""
Same precinct -> district cross-reference as distritos_analysis.py, but for
the council race (Vereador) instead of mayor, 2020 and 2024 only -- the only
two elections where the group ran an identifiable council slate (tagged by
coalition party, via SQ_CANDIDATO against data/candidatos_2020/2024, the
same NOSSOS_2020/NOSSOS_2024 party sets used in scripts/vereadores_analysis.py).
Counts the group's FULL slate (elected and not elected), not just the
council members who won a seat -- a candidate can pull real votes in a
district without personally winning a municipality-wide seat.

Produces:
  - output/votos_vereadores_grupo_por_distrito_pct.csv (also _abs.csv, _total.csv)
  - output/chart_distritos_vereadores_heatmap.b64
"""
import pandas as pd
import matplotlib
matplotlib.use("Agg")
...
# ---------------------------------------------------------------------------
# 1) precinct -> polling location (2024) -> district (same mapping as prefeito)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 2) group's full council slate per year, by SQ_CANDIDATO (party-coalition
#    membership, not just the ones who won a seat)
# ---------------------------------------------------------------------------
YEARS = [2020, 2024]
...
gvo = gvo.reindex(DISTRITO_ORDER)
tot = tot.reindex(DISTRITO_ORDER)

pct.to_csv(os.path.join(OUT, "votos_vereadores_grupo_por_distrito_pct.csv"))
gvo.to_csv(os.path.join(OUT, "votos_vereadores_grupo_por_distrito_abs.csv"))
tot.to_csv(os.path.join(OUT, "votos_vereadores_total_por_distrito.csv"))
print("=== % de votos p/ vereador que foram na chapa do grupo, por distrito ===")
print(pct.to_string())

# ---------------------------------------------------------------------------
# Chart: heatmap, districts x years (2020, 2024), same red-green diverging
# palette as the prefeito heatmap (Figura 6)
# ---------------------------------------------------------------------------
...
fig.text(0.5, 0.97, "ONDE A CHAPA DE VEREADOR ERA FORTE", fontsize=14.5, color=TITLE_GRAY, fontweight="bold", fontfamily="Anton", ha="center", va="top")
fig.text(0.5, 0.905, "% dos votos p/ vereador na chapa do grupo, por distrito, 2020 e 2024", fontsize=9.5, color=MUTED, fontfamily="Anton", ha="center", va="top")
...
chart_path = os.path.join(OUT, "chart_distritos_vereadores_heatmap.b64")
